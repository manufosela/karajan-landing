...
template: splash
hero:
  tagline: "v2.10.2 — `kj init` wizard expanded: per-role provider selection (10 roles, inherit/pick CLI/disable), automatic SonarQube token bootstrap via REST API (no more web UI walk), git automation flags (auto_commit/push/pr) and HU Board security prompts. On top of the agent-readiness surface from v2.10: `llms.txt` index, a `SKILL.md` per CLI command under `docs/agents/`, and a static auditor (`kj audit --agent-readiness`) that scores any third-party repo 0–100 across seven LLM-free checks. Karajan-on-Karajan: 100/100. Webperf quality gate inside the iteration loop. HU Board hardening: binds 127.0.0.1 by default. a11y / WCAG / ARIA tasks auto-pull the frontend-ui-engineering skill. Asciinema demo scripts under `docs/demos/`. 4 375 tests across 374 files."
  image:
