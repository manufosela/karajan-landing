...
template: splash
hero:
  tagline: "v2.7.3 — Task from a .md file (kj run --task-file spec.md, same on MCP). CLI finally writes .kj/run.log like MCP does, so kj-tail works whether Claude Code launches kj via Bash or MCP. Node 18 LTS really supported now. kj-tail v1.38.0 waits for the log instead of exiting. Plus full v2.7 skills story: addyosmani/agent-skills first-source, summary.md surfaces slugs per role. 3 702 tests, 287 files."
  image:
