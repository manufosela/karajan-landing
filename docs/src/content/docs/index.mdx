...
<CardGrid stagger>
  ...
  <Card title="5 700+ Automated Tests" icon="approve-check">
...
### v3.4.2 — Post-Phase-1 hardening + install hotfix

A maintenance line on top of v3.4.0. Three observability fixes found while measuring Phase 1, the top of the v3.4.0 self-audit (run with claude-fable-5), and — most importantly — a **pre-publish gate** so a broken artifact can never ship again.

**Observability & audit follow-ups**: the session journal (Cache hits, budget, commits) is now written on **every** run ending, not just approved ones (KJC-BUG-0084); the audit role reports its cached tokens (it was the most expensive role and reported 0 — KJC-BUG-0085); a preflight check fails fast with an actionable message when Sonar cannot derive a project key, instead of burning iterations and dying in `sonar_repeat` (KJC-BUG-0083). Plus a shared `escapeRegExp()`, 23 dead exports pruned, four complex regexes decomposed, and the HU Board API with zero sync fs calls + mtime-cached plan scans.

**The install hotfix (v3.4.1 → v3.4.2)**: v3.4.1 shipped unable to run `kj --version` — `@karajan/core` dragged `sqlite-vec` into the bundle, and sqlite-vec's `files: []` left it without its entry point. Fix: core's runtime deps are now `peerDependencies`, resolved complete from the consumer's top-level install (KJC-BUG-0086). v3.4.1 is deprecated on npm.

**Why it won't happen again**: the published tarball is now packed, installed clean in isolation, and actually run (`kj --version` / `--help`) **before every publish** via a `prepublishOnly` gate, and on every PR via the `pack-smoke` CI job (KJC-TSK-0553). The test suite proved the linked workspace; this proves the artifact users receive.

**Upgrade from any 3.x** — v3.4.2 is the first fully-clean 3.x npm install.

### v3.5.0 — Quality harness: kj harden + kj check (current)

The **quality harness** epic (KJC-PCS-0059): the guardrails Karajan was built with, installable into any repo — new or existing — in one command, then verifiable as a CI drift gate.

**`kj harden`** is idempotent, stack-aware, and never overwrites content outside its `kj:managed` markers. It installs git hooks under `.karajan/hooks/` (via `core.hooksPath`, so it never fights husky), config seeded if absent (`.editorconfig`, `commitlint`, and per-language lint/format — eslint with an ES2025 deprecated-API blacklist + prettier for JS/TS, `ruff` for Python, `.golangci.yml` for Go), CI workflows (AI-attribution gate, stack-aware Quality, shrink-budget on strict, pack-smoke on publishable npm), and agent guidelines into `AGENTS.md`/`CLAUDE.md`.

**The key design choice**: hook lint/format/test commands are **native to the detected language**, and the commit-msg hook is pure POSIX. So hardening a Go, Python or Java repo **never drags Node onto a contributor's machine** — Node is only needed once, by whoever runs `kj harden`. In a fullstack monorepo each language root gets its own config.

**`kj check`** verifies the harness is present and intact (hooks executable + marked, `core.hooksPath`, config/workflows present), with exit 0/≠0 and `--json` for CI. It catches drift — a deleted hook, a stripped marker — and the greenfield gap: a language you added *after* hardening whose config was never seeded.

**`kj init`** now runs the same engine, so a freshly initialised project is hardened out of the box (`--no-harden` to opt out). 5 717 tests passing; clean install verified.

**Why a minor**. Additive feature, no breaking changes.
