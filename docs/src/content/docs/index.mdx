...
template: splash
hero:
  tagline: "v2.7.4 — Sonar is intrinsic to Karajan for code tasks (like TDD). `sonarqube.enabled` and `--no-sonar` are ignored with a deprecation warning — a code task without a quality gate is not a job Karajan calls complete. Kills the bogus API-key preflight check that was blocking every Claude Code MCP run. Two new architectural invariants (`no-provider-apis`, `sonar-intrinsic`) enforce these contracts in CI. New `docs/TESTS.md` guide with diagrams. 3 720 tests, 289 files."
  image:
