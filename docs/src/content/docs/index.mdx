...
<CardGrid stagger>
  ...
  <Card title="5 387+ Automated Tests" icon="approve-check">
    458 test files covering every pipeline role, guard, config option, MCP tool, and the full HU Board package. Full suite runs in around 60 s with Vitest. Opt-in subsystems (brain, ci, sonar, hu-board, webperf) are labelled `[opt-in: <feature>]` so fast feedback loops can skip them via `KJ_SKIP_ALL_OPTIN=1`.
  </Card>
