...
<CardGrid stagger>
  ...
  <Card title="5 436+ Automated Tests" icon="approve-check">
...
**Coming in v2.26.0+** — **chokidar watcher** for live re-indexing without on-demand `kj rag index`, **AST-aware source chunker** (tree-sitter or @babel/parser) to replace the export-symbol regex, **BM25 + cosine hybrid scoring** for better precision on keyword-heavy queries.

### v2.25.0 — Minor (current)
...
**Coming in v2.26.0+** — chokidar watcher (live re-indexing), AST source chunker, BM25 + cosine hybrid scoring.
