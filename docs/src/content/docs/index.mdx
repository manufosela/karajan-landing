...
<CardGrid stagger>
  ...
  <Card title="5 414+ Automated Tests" icon="approve-check">
...
**Coming in v2.25.0+** — **Camino B** (`/kj-rag-query` slash command for Skills hosts without MCP), **Camino D** (Brain decisor heuristic that decides **when** to pre-fetch — refinement on top of C, e.g. skip for trivial tasks, fetch with topK=10 for complex multi-HU plans), **chokidar watcher** for live re-indexing without on-demand `kj rag index`, **AST-aware source chunker** (tree-sitter or @babel/parser) to replace the export-symbol regex, **BM25 + cosine hybrid scoring** for better precision on keyword-heavy queries.
