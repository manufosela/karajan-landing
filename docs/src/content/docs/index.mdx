...
<CardGrid stagger>
  ...
  <Card title="5 720+ Automated Tests" icon="approve-check">
    468 test files covering every pipeline role (incl. the new rag-context-stage), guard, config option, MCP tool (27 total), the full HU Board package, and the Project RAG subsystem. Full suite runs in around 60 s with Vitest. Opt-in subsystems (brain, ci, sonar, hu-board, webperf) are labelled `[opt-in: <feature>]` so fast feedback loops can skip them via `KJ_SKIP_ALL_OPTIN=1`.
  </Card>
...
**Coming in v2.29.0+** — extended cloud embedder providers (Cohere, Anthropic via OAuth), per-chunk metadata search, rerank with cross-encoder, retrieval-quality dashboard on the HU Board.

### v2.28.0 — Minor (current)
...
**Coming in v2.29.0+** — extended cloud providers (Cohere, Anthropic via OAuth), per-chunk metadata search, rerank with cross-encoder, retrieval-quality dashboard on the HU Board.
