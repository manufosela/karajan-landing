...
template: splash
hero:
  tagline: "v1.34.0 — Role-based AI orchestrator: 15 roles, 5 agents, deterministic guards. Code, review, test, secure — in a single command."
  image:
...
<CardGrid stagger>
  ...
  <Card title="MCP Server" icon="puzzle">
    20 tools exposed via MCP — including `kj_discover`, `kj_triage`, `kj_researcher`, `kj_architect` for standalone role execution, `kj_preflight` for human-confirmed agent config, and `kj_status` for live parsed status. Real-time progress notifications for all tools.
  </Card>
  ...
  <Card title="1920+ Automated Tests" icon="approve-check">
