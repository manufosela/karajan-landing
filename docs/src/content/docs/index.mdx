...
template: splash
hero:
  tagline: "v2.10.1 — Patch on top of the agent-readiness release. Karajan is the first orchestrator with a full agent-readability surface: an `llms.txt` index at the root, a `SKILL.md` per CLI command under `docs/agents/`, and a static auditor (`kj audit --agent-readiness`) that scores any third-party repo 0–100 across seven LLM-free checks (llms.txt, robots AI-bot allowlist, page token budgets ≤ 32 KB, heading hierarchy, agents/README, SKILL.md coverage). Karajan-on-Karajan: 100/100. Six new SKILL.md files (`kj doctor / init / board / review / resume / clean`) with CI guard. Webperf quality gate inside the iteration loop. HU Board hardening: binds 127.0.0.1 by default, `--bind 0.0.0.0` enforces auto-generated token + helmet + rate-limit. a11y / WCAG / ARIA tasks auto-pull the frontend-ui-engineering skill. Asciinema demo scripts under `docs/demos/`. 4 359 tests across 373 files."
  image:
