...
template: splash
hero:
  tagline: "v2.11.0 — Dogfooding pass release. Two-day pass through a 10-level test plan surfaced and fixed a long tail of UX papercuts and three latent bugs that only show up on fresh `/tmp` repos. The `SonarStage` no longer loops on remoteless projects (was burning iterations until `max_iterations`-fallback-approval); the post-loop `commitAll` tolerates the locale-specific 'nothing to commit' race; the HU sub-pipeline branches off `master`/`HEAD` when the configured `main` doesn't exist; and `runFlow` seals `session.status` at the boundary so `kj status` never shows zombi `running` runs. Plus `hu-board` gains automatic ephemeral-project cleanup at boot and an in-UI help modal for the five views. 14 PRs (#624–#637), 4 452 tests across 377 files."
  image:
