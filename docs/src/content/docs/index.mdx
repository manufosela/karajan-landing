...
### v3.7.2 — Deep-link fix + resilient audit fallback (current)
...
### v3.5.1 — First-class PHP in the harness

PHP joins JS/TS, Python and Go as a first-class `kj harden` language: `composer.json` detection, native hooks (`phpstan` / `php-cs-fixer` / `phpunit`), a `phpstan.neon` config and a `setup-php` Quality CI workflow — native tooling, no npm/Node imposed at commit time.

### v3.5.0 — Quality harness: kj harden + kj check

`kj harden` installs the whole quality harness — git hooks, lint/format/commit config, CI quality gates and agent guidelines — into any repo in one command, idempotent and stack-aware behind `kj:managed` markers that never overwrite your content. Native per-language hook commands keep Node off non-JS contributors' machines. `kj check` verifies it as a CI drift gate; `kj init` installs it out of the box.

### v3.4.2 — Post-Phase-1 hardening + install hotfix

A pre-publish gate packs the real tarball, installs it clean and isolated, and runs it (`kj --version`) before every publish, so a broken artifact can never ship again. Plus three observability fixes and the v3.4.1 install hotfix (`@karajan/core` runtime deps → `peerDependencies`).

### v3.4.0 — Cache-friendly prompts

Every prompt is split into a stable block (rendered first so prefix caching hits it) and a volatile tail. Measured: cold-run cache jumps from 47% to 99.6% and coder cost drops 76%.
