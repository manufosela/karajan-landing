...
template: splash
hero:
  tagline: "v2.7.0 — addyosmani/agent-skills as first-source process catalog. Karajan now pulls 20+ lifecycle skills (TDD, code-review, security, performance, git-workflow…) from Addy Osmani's curated repo and injects them per role BEFORE OpenSkills stack skills. Shallow clone + weekly refresh + graceful degradation when git is missing. 3 672 tests, 285 files."
  image:
