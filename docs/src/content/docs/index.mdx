...
### v3.10.1 — macOS binary + kj run on the standalone (current)
...
### v3.8.0 — kj start + inbound secret redaction

Minor. **`kj start`** is the single door to Karajan's autonomous squad: you state an intent (and at most your project's maturity — new, existing or legacy) and the Brain runs the whole read-only sweep for you (`doctor` / `check` / `harden` / `onboard`), then proposes what to do next. A cheap AI layer decides the *what* from a closed set and maps it to a saved command, always asking before anything that writes — the model never runs commands on its own, and you never have to learn the underlying subcommands. Plus **inbound secret redaction**: a hardcoded credential in your working tree is now masked before the diff reaches the (possibly cloud) reviewer model, keeping an identifiable prefix (`AKIA***REDACTED***`) so the reviewer can still say "move this to .env" without ever seeing the value. And `kj advanced` keeps `kj --help` focused on the core commands.

### v3.7.2 — Deep-link fix + resilient audit fallback

Patch. The HU Board auto-start banner now emits the canonical hash route (`/#board/<slug>`) the SPA router understands, so `kj plan` / `kj run` deep-links land on your project instead of the global dashboard. And `kj audit` is now resilient: if the configured provider or model is down, it automatically walks an ordered fallback chain — configured provider+model → the same provider's default model → the remaining known providers (claude, codex, gemini) — and the first one that works wins, so a dead model can no longer kill the command.

### v3.7.1 — init fix + pnpm safety

Patch. Fixes a `kj init` crash when configuring the Plan B fallback, and makes pnpm installs safe: pnpm blocks dependency build scripts by default, so `better-sqlite3`'s native addon wouldn't compile — `kj doctor` now detects this and tells you exactly how to fix it (`pnpm approve-builds better-sqlite3`, or install with npm). The release gate checks pnpm on every version too.

### v3.7.0 — Autonomous delivery

A single autonomy axis (`interactive | assisted | autonomous`) lets Karajan run hands-off. **`kj autorun <spec>`** chains plan → run → outcome report: it plans, decomposes into HUs, and runs to completion with no human in the loop. **The Arbiter** resolves agent conflicts by picking the least-bad answer (ground truth: acceptance tests > reviewer must-fix > nice-to-have); autonomous stages never block, a wall-clock backstop prevents hangs, and the run ends with a DELIVERED / INCOMPLETE report of residual defects. The default stays `interactive`, so normal runs are untouched.

### v3.6.0 — Advisory harden

`kj harden --report` shows, read-only, what the kj standard would add to an existing repo — per artifact: missing / yours / kj-managed, plus the concrete improvements for your own configs. `kj harden --interactive` adopts the standard piece by piece, default-safe. Scope flags (`--only`/`--exclude`) keep it off demo/fixture dirs. A rounder `kj init` first run (consistent English, glossed terms, clean close), and the `kj-trash` safety net now ships to npm installs.
