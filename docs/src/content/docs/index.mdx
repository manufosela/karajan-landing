...
template: splash
hero:
  tagline: "v2.8.0 — Audit-driven hardening release. Closed all 13 items the 2026-04-30 self-audit flagged across security, code quality, performance, architecture and testing in 16 PRs. Every `child_process` call in src/ now uses tokenised arg arrays (no shell expansion). FASE 1 e2e suite (7 scenarios) covers the 5-bug class from the 2026-04-27 demo regression. Major file splits: cli.js 699→113 LOC, plan.js 549→14, iteration-loop.js 513→311, pre-loop.js 626→435. ESLint ratcheted: 57 warnings closed, key rules switched warn→error. BREAKING: engines.node 18→20.10.0 (Node 18 LTS hit EOL on 2025-04-30). 4 199 tests across 357 files."
  image:
...
<CardGrid stagger>
  <Card title="Audit-Driven Hardening (v2.8)" icon="seti:lock">
    Every `child_process` call in `src/` migrated to tokenised arg arrays — no shell metacharacter expansion anywhere, even with constant inputs. `cli.js`, `plan.js`, `iteration-loop.js` and `pre-loop.js` split along clean seams; each driver now under the 600-LOC ceiling. New ESLint rules ban `console.*` outside CLI paths and `globalThis.__KJ_*` outside the test-harness. 57 warnings cleared in `src/`, then `no-unused-vars` / `no-useless-assignment` / `preserve-caught-error` ratcheted to errors so regressions fail CI.
  </Card>
  <Card title="FASE 1 E2E Suite (v2.8)" icon="approve-check">
    7 scenarios + fake-coder/sonar fixtures cover the 5-bug class from the 2026-04-27 demo regression: zombie-HU, saveSession-missing, Repairer unfixable, zombi-status, kj-audit smoke. Each test < 90s; full e2e suite runs in 6s with no real LLM/network. The bugs that took down the demo last week now fail CI before merge.
  </Card>
