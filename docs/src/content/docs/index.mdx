...
### v3.5.1 — First-class PHP in the harness (current)
...
Cache metrics normalized across Anthropic, OpenAI/Codex, Gemini, aider and opencode, surfaced as a `🎯 N%` badge on the HU Board, persisted in `board.db` and emitted in `pipeline_complete` telemetry.
