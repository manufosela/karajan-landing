...
template: splash
hero:
  tagline: "v2.9.0 — Audit overhaul release. `kj audit` becomes a stack-aware, two-phase analysis tool. Deterministic findings (basalCost + Sonar + OSV-Scanner + Semgrep + WebPerf) print first; an interactive prompt lets you decide whether to spend tokens on the LLM phase. New `--deterministic-only` flag for zero-token runs, `--report-file` to persist audits to .md or .json, `## LLM Usage` section showing tokens + cost in USD. Three new deterministic security collectors: SonarQube findings as ground truth, OSV-Scanner for CVE coverage broader than `npm audit`, Semgrep for SAST (XSS, SQLi, taint flow — equivalent to `snyk code` but free and local). Stack-aware prompt filters dimension heuristics — no more N+1 nags on Astro projects. New accessibility dimension auto-active for frontend (WCAG 2.x). CLI/MCP parity bug fixed. 228 dead exports cleaned in 5 atomic PRs. 4 305 tests across 367 files."
  image:
