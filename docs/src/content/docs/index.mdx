...
template: splash
hero:
  tagline: "v2.10.0 — Agent-readiness release. Karajan is now the first orchestrator with a full agent-readability surface: an `llms.txt` index at the root, a `SKILL.md` per CLI command under `docs/agents/`, and a static auditor that scores any third-party repo against the same shape. New: `kj audit --agent-readiness` produces a 0–100 score across seven LLM-free checks (llms.txt, robots AI-bot allowlist, page token budgets ≤ 32 KB, heading hierarchy, agents/README, SKILL.md coverage). Karajan-on-Karajan: 100/100. Six new SKILL.md files (`kj doctor / init / board / review / resume / clean`) with CI guard. Webperf quality gate inside the iteration loop. HU Board hardening: binds 127.0.0.1 by default, `--bind 0.0.0.0` enforces auto-generated token + helmet + rate-limit. a11y / WCAG / ARIA tasks now auto-pull the frontend-ui-engineering skill. Asciinema demo scripts under `docs/demos/`. 5 PRs (#605–#610). 4 358 tests across 373 files."
  image:
