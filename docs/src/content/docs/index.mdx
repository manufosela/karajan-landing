...
<CardGrid stagger>
  ...
  <Card title="4 835+ Automated Tests" icon="approve-check">
    400 test files covering every pipeline role, guard, config option, and MCP tool. Full suite runs in around 60 s with Vitest. Opt-in subsystems (brain, ci, sonar, hu-board, webperf) are labelled `[opt-in: <feature>]` so fast feedback loops can skip them via `KJ_SKIP_ALL_OPTIN=1`.
  </Card>
