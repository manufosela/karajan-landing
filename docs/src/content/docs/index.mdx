...
### v4.14.0 — the panel never runs dry (current)
...
### v4.10.0 — nothing personal ships

Minor closing every outbound boundary. Born from a real incident — personal emails published on a landing page inside release notes — the Privacy Gate audits everything that leaves the machine: the **pre-commit** rejects a staged diff carrying your denylisted data (masked, with a named escape), **verify-pack** scans the exact tarball that would publish, and `kj privacy scan <dir>` audits any build output before deploy. Two detection layers ride every boundary: your personal denylist (`kj env install` asks and writes it — you never fill YAML) and pattern detectors — generic PII plus **hardcoded secrets**: a platform token (`ghp_`, `sk-`, `AKIA`…) blocks outright, a `password: "literal"` warns you toward `.env`. Findings never echo the datum they flag. Also in this release, a field lesson turned structural: **installing IS activating** — `kj env install` now performs the enforcement itself (git hooks, verdict gate) instead of trusting the agent to run the setup steps, and ends by printing the method into the very conversation that installed it. A commit outside the method is rejected, not narrated.

### v4.9.0 — excellent code is a choice, not inertia

Minor opening the excellent-code track. On legacy codebases agents optimize for local coherence — improving while following the existing line — even when the canonical solution is better; nothing in the loop asked for the alternative. Now the method does: every non-trivial plan names at least two approaches, **one as if the codebase didn't exist**, and says why the winner won. And the architect answers that question with a citable canon: `kj rag query --library` serves distilled pattern cards — problem signature, when it beats the default, *when NOT to apply it*, and the canonical citation (Lamport ships first; the rest of the canon lands next). Following the legacy line is a choice, never a default.

### v4.8.0 — the security pass an agent self-invokes
...
### v4.7.0 — least privilege for agents
...
### v4.6.2 — warnings stop being negotiable
