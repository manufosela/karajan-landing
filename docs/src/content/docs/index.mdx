...
template: splash
hero:
  tagline: "v2.13.0 — HU Board hardening. Five PRs absorb the pathologies a dogfooding session uncovered: a prompt modal from days earlier blocking the entire UI, 18 zombi projects reappearing after every `kj board start`, the browser serving stale HTML/JS after a server restart, and the prompt modal showing transparent cards underneath because `var(--bg-secondary)` was never declared. Now: tombstones persist deletes across `fullScan`; new endpoints `DELETE /api/prompts/:id`, `DELETE /api/plans/:planId`, `GET /api/tombstones`, `POST /api/tombstones/.../restore`; new command `kj board cleanup` sweeps ephemeral projects, orphan prompts and dangling session dirs in one pass; `Cache-Control: no-store` + `/api/version` polling auto-reloads the client when boot time changes; new 🧹 button as manual escape hatch; CSS `--bg-secondary` finally declared so modals are opaque. 5 PRs (#654–#658). 4 522 tests across 381 files."
  image:
