...
### v3.11.0 — kj install-tools ya instala de verdad (actual)

Minor. `kj install-tools` se convierte en el **actuador**: ahora instala las herramientas externas de auditoría en lugar de solo nombrarlas (`kj doctor` se queda como el diagnosticador que te dice qué falta). Un asistente muestra el **comando exacto por herramienta** e instala al aceptar — los pasos con privilegios ejecutan `sudo` en tu propia terminal, así que kj nunca ve ni registra la contraseña, y los binarios se descargan a un fichero temporal, `chmod` y renombrado atómico (sin artefacto a medio instalar si algo falla). Sin gestor de paquetes, osv-scanner tira de su binario estático de GitHub y semgrep cae a un comando concreto de Docker/pipx; Docker en Linux prefiere `apt`/`dnf` o descarga el script oficial a un fichero y lo ejecuta (nunca `curl | sh`), siempre opt-in y con el comando mostrado primero. Además, tres refinamientos de rol — disensión de socio activo, un micro-gate de alineación y reformulación en positivo de instrucciones de alto impacto — y dos arreglos de instalación (el hook post-merge del RAG respeta `core.hooksPath`; el instalador de macOS elige el binario darwin-arm64). Actualización directa desde v3.10.2.

### v3.10.2 — Privacidad, contención de rutas y observabilidad del triage

Patch. Tres endurecimientos pequeños agrupados. Un email personal que se había colado en los metadatos públicos del paquete se **sustituye por el handle público**, para que los scrapers no puedan cosecharlo. Las escrituras de fichero de acción directa del Brain confinan ahora cada ruta al proyecto con una **comprobación de contención** (basada en `path.relative`, no en prefijo de cadena), así que una ruta hermana manipulada como `../project-evil` no puede escapar del repo. Y el **triage ahora se explica**: en lugar de un único bloque global de razonamiento, emite una justificación por rol — para cada rol, si se activó o se omitió y por qué — mostrada en vivo para que cada decisión sea observable. Actualización directa desde v3.10.1.

### v3.10.1 — Binario de macOS + kj run en el standalone
