...
template: splash
hero:
  tagline: "v2.9.0 — Release de overhaul del audit. `kj audit` se convierte en una herramienta de análisis stack-aware con flujo en dos fases. Los hallazgos deterministas (basalCost + Sonar + OSV-Scanner + Semgrep + WebPerf) se imprimen primero; un prompt interactivo te deja decidir si pagar tokens en la fase LLM. Nuevo flag `--deterministic-only` para corridas sin tokens, `--report-file` para persistir audits a .md o .json, sección `## LLM Usage` con tokens y coste en USD. Tres nuevos colectores deterministas de seguridad: hallazgos SonarQube como ground truth, OSV-Scanner para CVEs (DB más amplia que `npm audit`), Semgrep para SAST (XSS, SQLi, taint flow — equivalente a `snyk code` pero gratis y local). El prompt stack-aware filtra heurísticas — sin más alertas N+1 en proyectos Astro. Nueva dimensión accessibility auto-activa para frontend (WCAG 2.x). Bug de paridad CLI/MCP corregido. 228 dead exports limpiados en 5 PRs atómicas. 4 305 tests en 367 ficheros."
  image:
