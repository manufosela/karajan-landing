...
template: splash
hero:
  tagline: "Orquestador de coding multiagente. 16 roles, 5 agentes, guards deterministas, TDD, SonarQube, revisión automatizada."
  image:
...
<CardGrid stagger>
  ...
  <Card title="4 906+ Tests Automatizados" icon="approve-check">
    407 ficheros de test cubriendo cada rol, guard, opción de config y herramienta MCP. Suite completa en unos 60 s con Vitest. Los tests de subsistemas opt-in (brain, ci, sonar, hu-board, webperf) llevan la etiqueta `[opt-in: <feature>]` y se pueden saltar con `KJ_SKIP_ALL_OPTIN=1`.
  </Card>
