...
<CardGrid stagger>
  ...
  <Card title="5 700+ Tests Automatizados" icon="approve-check">
...
### v3.4.2 — Hardening post-Phase-1 + hotfix de install

Línea de mantenimiento sobre v3.4.0. Tres fixes de observabilidad detectados midiendo Phase 1, el top del self-audit de v3.4.0 (con claude-fable-5) y —lo más importante— un **gate pre-publish** para que nunca vuelva a salir un artefacto roto.

**Observabilidad y audit**: el session journal (Cache hits, budget, commits) se escribe en **todos** los finales de run, no solo los aprobados (KJC-BUG-0084); el rol audit reporta sus cached tokens (era el más caro y reportaba 0 — KJC-BUG-0085); un preflight falla rápido con mensaje accionable cuando Sonar no puede derivar el project key, en vez de quemar iteraciones y morir en `sonar_repeat` (KJC-BUG-0083). Más `escapeRegExp()` compartido, 23 dead exports podados, cuatro regex complejos descompuestos, y la API del HU Board sin llamadas fs síncronas + cache mtime de planes.

**El hotfix de install (v3.4.1 → v3.4.2)**: v3.4.1 salió sin poder ejecutar `kj --version` — `@karajan/core` arrastraba `sqlite-vec` al bundle, y su `files: []` lo dejaba sin entrypoint. Fix: las deps de runtime de core pasan a `peerDependencies`, resueltas completas desde el install del consumidor (KJC-BUG-0086). v3.4.1 está deprecada en npm.

**Por qué no se repetirá**: el tarball publicado se empaqueta, instala en limpio aislado y se arranca (`kj --version` / `--help`) **antes de cada publish** vía un gate `prepublishOnly`, y en cada PR vía el job de CI `pack-smoke` (KJC-TSK-0553).

**Upgrade desde cualquier 3.x** — v3.4.2 es el primer install npm de la línea 3.x completamente limpio.

### v3.5.0 — Harness de calidad: kj harden + kj check (actual)

La épica del **harness de calidad** (KJC-PCS-0059): los guardrails con los que se construyó Karajan, instalables en cualquier repo —nuevo o existente— en un comando, y verificables como gate de deriva en CI.

**`kj harden`** es idempotente, consciente del stack y nunca sobrescribe lo que hay fuera de sus marcadores `kj:managed`. Instala hooks de git bajo `.karajan/hooks/` (vía `core.hooksPath`, sin pelear con husky), config sembrada si falta (`.editorconfig`, `commitlint` y lint/formato por lenguaje — eslint con lista negra de APIs ES2025 obsoletas + prettier en JS/TS, `ruff` en Python, `.golangci.yml` en Go), workflows de CI (gate anti-atribución IA, Quality por stack, shrink-budget en strict, pack-smoke en paquetes npm publicables) y guías de agentes en `AGENTS.md`/`CLAUDE.md`.

**La decisión de diseño clave**: los comandos de lint/formato/test de los hooks son **nativos del lenguaje detectado**, y el hook commit-msg es POSIX puro. Así, endurecer un repo Go, Python o Java **nunca arrastra Node a la máquina de un contribuidor** — Node solo hace falta una vez, en quien ejecuta `kj harden`. En un monorepo fullstack cada raíz de lenguaje recibe su propia config.

**`kj check`** verifica que el harness está presente e íntegro (hooks ejecutables + marcados, `core.hooksPath`, config/workflows presentes), con exit 0/≠0 y `--json` para CI. Detecta deriva —un hook borrado, un marker eliminado— y el hueco del greenfield: un lenguaje que añadiste *después* de endurecer y cuya config nunca se sembró.

**`kj init`** ejecuta ahora el mismo motor, así que un proyecto recién inicializado queda endurecido de serie (`--no-harden` para desactivar). 5 717 tests verdes; install limpio verificado.

**Por qué un minor**. Feature aditiva, sin breaking changes.
