...
template: splash
hero:
  tagline: "v2.10.1 — Patch sobre la release agent-readiness. Karajan es el primer orquestador con una superficie completa de agent-readability: índice `llms.txt` en la raíz, un `SKILL.md` por cada comando del CLI bajo `docs/agents/`, y un auditor estático (`kj audit --agent-readiness`) que puntúa cualquier repo de terceros 0–100 sobre siete checks sin LLM (llms.txt, allowlist de bots IA en robots, presupuesto de tokens por doc ≤ 32 KB, jerarquía de headings, agents/README, cobertura de SKILL.md). Karajan sobre Karajan: 100/100. Seis nuevos SKILL.md (`kj doctor / init / board / review / resume / clean`) con guard de CI. Quality gate de webperf dentro del bucle de iteración. Hardening del HU Board: bindea 127.0.0.1 por defecto, `--bind 0.0.0.0` fuerza token autogenerado + helmet + rate-limit. Tareas a11y / WCAG / ARIA cargan automáticamente la skill frontend-ui-engineering. Scripts asciinema bajo `docs/demos/`. 4 359 tests en 373 ficheros."
  image:
