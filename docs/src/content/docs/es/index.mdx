...
template: splash
hero:
  tagline: "v1.34.2 — Orquestador de IA basado en roles: 15 roles, 5 agentes, guards deterministas. Código, revisión, tests, seguridad — en un solo comando."
  image:
...
<CardGrid stagger>
  ...
  <Card title="Servidor MCP" icon="puzzle">
    21 herramientas via MCP — incluyendo `kj_discover`, `kj_triage`, `kj_researcher`, `kj_architect` para ejecución standalone de roles, `kj_preflight` para configuración confirmada por humano, `kj_board` para gestión del HU Board, y `kj_status` para estado parseado en vivo. Notificaciones de progreso en tiempo real para todas las herramientas.
  </Card>
