...
### v3.10.1 — Binario de macOS + kj run en el standalone (actual)

Patch. El **binario standalone de macOS (Apple Silicon) ya se distribuye** en la release de GitHub, junto a los de Linux y Windows — así que en macOS puedes instalar `kj` directamente desde el binario o seguir usando `npm install -g karajan-code`; ambos canales sirven la v3.10.1. También corrige un fallo por el que `kj run` moría en el binario standalone con un error de símbolo ausente mientras `--version` seguía funcionando. Un smoke test de run-dispatch ahora protege cada release para que ese camino no pueda volver a romperse. Actualización directa desde v3.9.0.

### v3.9.0 — Binarios standalone

Minor. Karajan ahora se distribuye como un **único ejecutable standalone autónomo**. Los instaladores de una línea dejan `kj` en tu PATH: `curl -fsSL https://karajancode.com/install.sh | sh` en Linux e `irm https://karajancode.com/install.ps1 | iex` en Windows (PowerShell). Cada binario se construye a partir de un bundle SEA (Single Executable Application), se verifica contra su checksum SHA256 publicado y pasa por un smoke test que rechaza subir un binario cuyo `--version` no coincida con el tag. **macOS se instala por npm de momento** — el binario de Apple Silicon se está terminando aparte, así que en macOS `npm install -g karajan-code` sigue siendo la vía de instalación. Actualización directa desde v3.8.0.

### v3.8.0 — kj start + redacción de secretos inbound

Minor. **`kj start`** es la única puerta al squad autónomo de Karajan: expresas una intención (y como mucho la madurez de tu proyecto — nuevo, existente o legacy) y el Brain hace por ti todo el barrido en solo lectura (`doctor` / `check` / `harden` / `onboard`), y luego propone qué hacer a continuación. Una capa de IA barata decide el *qué* de un conjunto cerrado y lo mapea a un comando guardado, preguntando siempre antes de cualquier cosa que escriba — el modelo nunca ejecuta comandos por su cuenta, y tú nunca tienes que aprenderte los subcomandos de debajo. Además, **redacción de secretos inbound**: una credencial hardcodeada en tu working tree se enmascara ahora antes de que el diff llegue al modelo reviewer (posiblemente en la nube), conservando un prefijo identificable (`AKIA***REDACTED***`) para que el reviewer aún pueda decir "muévelo a .env" sin ver jamás el valor. Y `kj advanced` mantiene `kj --help` centrado en los comandos esenciales.

### v3.7.2 — Fix de deep-link + fallback resiliente del audit

Patch. El banner de arranque del HU Board emite ahora la ruta hash canónica (`/#board/<slug>`) que entiende el router de la SPA, así que los deep-links de `kj plan` / `kj run` aterrizan en tu proyecto y no en el dashboard global. Y `kj audit` es ahora resiliente: si el proveedor o el modelo configurado está caído, recorre automáticamente una cadena ordenada de alternativas — proveedor+modelo configurado → el modelo por defecto del mismo proveedor → el resto de proveedores conocidos (claude, codex, gemini) — y gana el primero que funciona, así que un modelo muerto ya no puede tumbar el comando.

### v3.7.1 — fix de init + seguridad pnpm

Patch. Arregla un crash de `kj init` al configurar el fallback Plan B, y hace seguras las instalaciones con pnpm: pnpm bloquea por defecto los build scripts de dependencias, así que el addon nativo de `better-sqlite3` no compilaba — `kj doctor` ahora lo detecta y te dice exactamente cómo arreglarlo (`pnpm approve-builds better-sqlite3`, o instalar con npm). El gate de release también verifica pnpm en cada versión.

### v3.7.0 — Entrega autónoma

Un único eje de autonomía (`interactive | assisted | autonomous`) deja que Karajan ejecute sin manos. **`kj autorun <spec>`** encadena plan → run → informe de resultado: planifica, descompone en HUs y ejecuta hasta terminar sin humano en el bucle. **El Arbiter** resuelve los conflictos entre agentes eligiendo la respuesta menos mala (verdad de base: tests de aceptación > must-fix del reviewer > nice-to-have); las stages autónomas nunca se bloquean, un tope de wall-clock evita los cuelgues, y la ejecución termina con un informe DELIVERED / INCOMPLETE de los defectos residuales. El default sigue siendo `interactive`, así que los runs normales no cambian.

### v3.6.0 — Harden consultivo

`kj harden --report` muestra, en solo lectura, qué aportaría el estándar kj a un repo existente — por artefacto: falta / es tuyo / gestionado por kj, más las mejoras concretas para tus propias configs. `kj harden --interactive` adopta el estándar pieza a pieza, con default seguro. Los flags de alcance (`--only`/`--exclude`) lo mantienen fuera de dirs de ejemplos/fixtures. Un primer uso de `kj init` más redondo (inglés consistente, jerga glosada, cierre claro), y la red de seguridad `kj-trash` ya llega a los installs de npm.
