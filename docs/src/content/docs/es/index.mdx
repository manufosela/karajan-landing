...
### v3.7.2 — Fix de deep-link + fallback resiliente del audit (actual)
...
### v3.5.1 — PHP de primera en el harness

PHP se suma a JS/TS, Python y Go como lenguaje de primera de `kj harden`: detección de `composer.json`, hooks nativos (`phpstan` / `php-cs-fixer` / `phpunit`), config `phpstan.neon` y un workflow Quality `setup-php` — tooling nativo, sin imponer npm/Node en el commit.

### v3.5.0 — Harness de calidad: kj harden + kj check

`kj harden` instala todo el harness de calidad —hooks de git, config de lint/formato/commits, gates de CI y guías de agentes— en cualquier repo en un comando, idempotente y consciente del stack, tras marcadores `kj:managed` que nunca sobrescriben lo tuyo. Los comandos de hook nativos por lenguaje mantienen Node fuera de la máquina de los contribuidores no-JS. `kj check` lo verifica como gate de deriva en CI; `kj init` lo instala de serie.

### v3.4.2 — Hardening post-Phase-1 + hotfix de install

Un gate pre-publish empaqueta el tarball real, lo instala limpio y aislado, y lo arranca (`kj --version`) antes de cada publish, para que un artefacto roto no pueda volver a salir. Más tres fixes de observabilidad y el hotfix de install de v3.4.1 (deps de runtime de `@karajan/core` → `peerDependencies`).

### v3.4.0 — Prompts cache-friendly

Cada prompt se parte en un bloque estable (renderizado primero para que el prefix caching lo pille) y una cola volátil. Medido: el cache en frío salta del 47% al 99,6% y el coste del coder baja un 76%.
