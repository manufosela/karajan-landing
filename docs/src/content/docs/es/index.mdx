...
<CardGrid stagger>
  ...
  <Card title="5 414+ Tests Automatizados" icon="approve-check">
...
**Viene en v2.25.0+** — **Camino B** (slash command `/kj-rag-query` para hosts Skills sin MCP), **Camino D** (heurística Brain decisor que decide **cuándo** pre-fetchear — refinamiento sobre C, ej. skipear en tasks triviales, fetchear con topK=10 en plans complejos multi-HU), **chokidar watcher** para re-indexing en vivo sin `kj rag index` on-demand, **chunker AST-aware** (tree-sitter o @babel/parser) reemplazando el regex de export-symbol, **scoring híbrido BM25 + cosine** para mejor precisión en queries keyword-heavy.
