...
template: splash
hero:
  tagline: "v2.11.0 — Release del paso de dogfooding. Dos días recorriendo un plan de pruebas de 10 niveles surfaron una larga cola de papercuts de UX y tres bugs latentes que sólo aparecen en repos `/tmp` recién creados. El `SonarStage` ya no entra en bucle en proyectos sin remote (antes quemaba iteraciones hasta el fallback de `max_iterations` aprobado); el `commitAll` del post-loop tolera la carrera locale-specific 'nada para hacer commit'; el sub-pipeline de HUs ramifica desde `master`/`HEAD` cuando el `main` configurado no existe; y `runFlow` sella `session.status` en la frontera, así que `kj status` ya no muestra runs zombi como `running`. Además, `hu-board` gana limpieza automática de proyectos efímeros al arrancar y un modal de ayuda integrado para las cinco vistas. 14 PRs (#624–#637), 4 452 tests en 377 ficheros."
  image:
