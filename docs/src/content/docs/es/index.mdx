...
template: splash
hero:
  tagline: "v2.8.0 — Release de endurecimiento dirigido por audit. Cerrados los 13 issues que el self-audit del 2026-04-30 detectó en seguridad, calidad de código, rendimiento, arquitectura y testing, en 16 PRs. Toda llamada a `child_process` en src/ ahora usa arrays de argumentos tokenizados (cero expansión de shell). Nueva FASE 1 e2e suite (7 escenarios) que cubre la familia de 5 bugs de la demo del 2026-04-27. Splits importantes: cli.js 699→113 LOC, plan.js 549→14, iteration-loop.js 513→311, pre-loop.js 626→435. ESLint ratcheteado: 57 warnings cerrados, reglas clave warn→error. BREAKING: engines.node 18→20.10.0 (Node 18 LTS llegó a EOL el 2025-04-30). 4 199 tests en 357 ficheros."
  image:
...
<CardGrid stagger>
  <Card title="Endurecimiento Dirigido por Audit (v2.8)" icon="seti:lock">
    Toda llamada a `child_process` en `src/` migrada a arrays de argumentos tokenizados — cero expansión de metacaracteres de shell, incluso con inputs constantes. `cli.js`, `plan.js`, `iteration-loop.js` y `pre-loop.js` divididos por costuras limpias; cada driver bajo el techo de 600 LOC. Nuevas reglas ESLint que prohíben `console.*` fuera de paths CLI y `globalThis.__KJ_*` fuera del test-harness. 57 warnings cerrados en `src/`, luego `no-unused-vars` / `no-useless-assignment` / `preserve-caught-error` ratcheteados a errores: las regresiones rompen CI.
  </Card>
  <Card title="FASE 1 E2E Suite (v2.8)" icon="approve-check">
    7 escenarios + fixtures fake-coder/sonar cubren la familia de 5 bugs de la demo del 2026-04-27: zombie-HU, saveSession-missing, Repairer unfixable, zombi-status, kj-audit smoke. Cada test < 90s; suite e2e completa en 6s sin LLM ni red reales. Los bugs que tumbaron la demo la semana pasada ahora rompen CI antes de poder mergear.
  </Card>
