...
template: splash
hero:
  tagline: "v2.10.2 — Wizard `kj init` ampliado: selección de provider por rol (10 roles, heredar/elegir CLI/desactivar), bootstrap automático del token de SonarQube via REST API (sin pasar por la UI web), flags de automatización git (auto_commit/push/pr) y prompts de seguridad del HU Board. Sobre la superficie agent-readiness de v2.10: índice `llms.txt`, un `SKILL.md` por comando del CLI bajo `docs/agents/`, y un auditor estático (`kj audit --agent-readiness`) que puntúa cualquier repo de terceros 0–100 sobre siete checks sin LLM. Karajan sobre Karajan: 100/100. Quality gate de webperf dentro del bucle de iteración. Hardening del HU Board: bindea 127.0.0.1 por defecto. Tareas a11y / WCAG / ARIA cargan automáticamente la skill frontend-ui-engineering. Scripts asciinema bajo `docs/demos/`. 4 375 tests en 374 ficheros."
  image:
