...
template: splash
hero:
  tagline: "v1.25.0 — Orquestador de IA basado en roles: 14 roles, 5 agentes, guards deterministas. Código, revisión, tests, seguridad — en un solo comando."
  image:
...
<CardGrid stagger>
  ...
  <Card title="Servidor MCP" icon="puzzle">
    19 herramientas via MCP — incluyendo `kj_discover`, `kj_triage`, `kj_researcher`, `kj_architect` para ejecución standalone de roles, `kj_preflight` para configuración confirmada por humano y `kj_status` para estado parseado en vivo. Notificaciones de progreso en tiempo real para todas las herramientas.
  </Card>
  ...
  <Card title="Pipeline Zero-Config" icon="setting">
    Auto-detecta TDD según el framework de tests del proyecto. Gestiona automáticamente el ciclo de vida Docker de SonarQube y la generación de config. Omite sonar/TDD para tareas de infra y doc automáticamente.
  </Card>
  <Card title="1605+ Tests Automatizados" icon="approve-check">
