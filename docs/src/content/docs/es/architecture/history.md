...
## Fase 106: v4.6.0 — El método, aplicado (v4.6.0)

El trade de la v4, nombrado y cerrado. v3 mandaba pero no pensaba — se embuclaba, le faltaba juicio; v4 piensa pero narra — un agente inteligente puede contar la historia de una regla en vez de seguirla, y el campo lo demostró tres veces en una semana. La respuesta es una escalera que cada regla del método sube hasta su techo determinista: texto (narrable) → rastro (infalsificable) → empujón (fricción en el momento) → gate (estructuralmente insaltable). La v4.6.0 sube cuatro reglas de golpe: **card-first se hace gate** — con el board integrado, una rama sin card viva no puede commitear; los boards externos reciben chequeo de presencia; **tests-con-código se hace gate** — fuentes sin un solo cambio de test avisan o bloquean; **worktree-por-tarea y tamaño de PR se hacen empujones** — el comando exacto en el momento exacto de la desviación; y **la adherencia se hace informe** — `kj check` muestra commits sin card, veredictos por workspace y commits sin tests, porque las desviaciones individualmente legítimas agregan una deriva visible. Toda escotilla de escape es explícita, con nombre y con registro. La regla de diseño hacia delante: una regla del método nace con su gate o su rastro — el texto es el recordatorio, nunca la garantía.

## Fase 107: v4.7.0–v4.8.0 — Mínimo privilegio, y la pasada de seguridad que el agente se auto-invoca

Un procesador de pagos publicó cómo opera agentes de IA de forma segura, y sus principios mapeaban casi uno a uno con los gates de Karajan — salvo tres, cerrados en dos releases. **Los agentes reciben solo lo que su función requiere**: los subprocesos lanzados reciben un allowlist de entorno (las familias de auth de su propio CLI, jamás tus claves cloud — con un contrato arbitrado por Solomon que decide cuándo un env explícito es completo o un overlay), y `kj check` mantiene un inventario de cada MCP alcanzable del proyecto, marcando lo aparecido desde el último check — un inventario que nadie reconcilia es la sensación de control sin el control. Y **la seguridad se vuelve autoservicio**: `kj audit --security` corre una pasada determinista de cero tokens — prompt-injection sobre la superficie de contexto del agente (los ficheros de reglas, templates y specs que entran a su contexto cada sesión), OSV, Semgrep, Sonar — ordenada por el playbook cuando una tarea toca auth, entrada de usuario, secretos, red o dependencias: diseñar → escanear → remediar, en la misma sesión.

## Fase 108: v4.9.0 — El código excelente es una elección, no inercia

Ante legacy, los agentes optimizan coherencia local — mejorar siguiendo la línea existente — incluso cuando la solución canónica es mejor; nada en el bucle pedía la alternativa. Ahora el método sí, por partida doble. La **cláusula de alternativas**: todo plan no trivial nombra al menos dos enfoques, uno respondiendo a "¿qué construirías si este código no existiera?", y justifica por qué gana el elegido — seguir la línea del legacy es una elección, nunca un default. Y la **library**: un canon de ingeniería destilado como colección propia del RAG — fichas de patrón con la firma del problema, cuándo gana al default, cuándo NO aplicarlo (la mitad del valor) y la cita canónica — para que el architect ancle esa alternativa greenfield en canon citable y no en intuiciones. Lamport estrenó; el canon crecerá con el radar de la familia como futuro alimentador.

## Fase 109: v4.10.0–v4.11.0 — Nada personal se publica, e instalar ES activar

Nacida de dos casos de campo en un fin de semana. Un incidente real (correos personales publicados en una landing dentro de unas release notes) se convirtió en la **Privacy Gate**: cada boundary de salida — diff staged, tarball de npm, cualquier build — se audita antes de dejar la máquina, con dos capas en cada uno: tu denylist personal (preguntada y escrita por la propia instalación, jamás YAML a mano) bloquea; el PII genérico y los tokens de plataforma hardcodeados (`ghp_`, `sk-`, `AKIA`…) bloquean o avisan hacia el `.env`. Los hallazgos jamás ecoan el dato que señalan. Y una sesión que "instaló karajan" y luego escribió una feature entera a mano destapó el agujero más profundo: **la propia instalación era narrable** — los pasos de setup eran texto que un agente podía saltarse. Ahora `kj env install` ejecuta él mismo el enforcement (hooks de git, gate de veredicto, y un tool gate que bloquea `Write` sobre ficheros existentes en el momento de invocar la herramienta), imprime el método en la propia conversación que instaló, cablea el RAG como herramienta MCP nativa, y pregunta qué board gobierna en vez de asumir en silencio. Un commit fuera del método se rechaza, no se narra — y una instalación, también.

## Fase 110: v4.12.0–v4.13.0 — El check es la garantía, y el programa manda

Dos respuestas a la misma crítica: *"da igual lo que apuntes, pasado un tiempo lo ignoras."* Primero la ceremonia de release dejó de depender de la memoria — `kj release check` la verifica determinista (manifest contra CHANGELOG, tags, escaneo de privacidad de los ficheros publicables exactos, más las verdades declaradas de cada proyecto, como "la web desplegada muestra esta versión"). Después la crítica llegó a su forma más profunda: un agente como brain no puede ser estricto con sus propias reglas. El **Karajan Sentinel** (épica completa en un día, v4.13.0) restauró el poder que faltaba: un supervisor determinista con cero LLM y autoridad síncrona sobre el agente a través de los hooks del harness. Estado del método registrado por sesión; operaciones sensibles bloqueadas ANTES del daño (editar fuentes sin card, publicar con el release check en rojo, empujar con violaciones abiertas); y un hook Stop que mantiene el turno abierto hasta resolver las violaciones. Autoprotegido — sus scripts se verifican contra el paquete kj instalado como raíz de confianza, y cada escape se audita de vuelta al usuario — y honesto con su frontera: los hooks síncronos con bloqueo solo existen en Claude Code, así que el nivel garantizado nombra a su anfitrión. La v3 tenía autoridad sin inteligencia; la v4 inteligencia sin autoridad; el Sentinel separa los poderes: el programa manda, el agente piensa.

## Fase 111: v4.14.0 — El panel nunca se seca

Esta fase la escribió un apagón real: codex agotó su cuota semanal en mitad de una tarea, y todos los recambios que el método daba por hechos resultaron muertos o en conflicto a la vez. La respuesta fue estructural, no circunstancial. Un registro declarativo de candidatos a reviewer — cada uno con su tier, sus heurísticas de auth y su comando de login exacto — alimenta el **failover de cuota con consentimiento**: cambio a un candidato autenticado avisando en alto, o el menú al usuario; jamás silencio, jamás auto-revisión. El propio panel creció: Kimi Code (tier gratis) y Antigravity CLI (el linaje gemini de vuelta, bajo Google AI Pro) entraron como octavo y noveno agentes integrados, cada uno verificado contra su CLI vivo antes de escribir una línea de adaptador — y el estreno destapó dos veces en dos días la misma clase de bug real (el pin de modelo de un rol viajando a un proveedor que no lo sirve), ahora cerrada de raíz. Y limpieza con dientes: packages/ entró en la superficie de lint tras pudrirse hasta 1.124 errores precisamente porque nadie miraba — y el script de lint hace ese deterioro imposible desde ya.

## Fase 112: v4.15.0 — El torneo gobernado
