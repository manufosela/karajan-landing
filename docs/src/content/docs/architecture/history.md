...
## Phase 112: v4.15.0 — The governed tournament

The question this phase answers: if several AIs can code, why does only one get the task? `kj tournament` fans the SAME task out to N coders, each in an isolated worktree lane leaving verifiable per-lane evidence. Then the funnel narrows deterministically: `--score` builds a zero-LLM scoreboard — a red suite eliminates on the spot, and the lexicographic ranking rule is printed with the result — `--judge` brings a cross-AI judge with no conflict of interest (a participant never judges its own race; a tie or a disagreement escalates to solomon), and `--crown` promotes the winner through the NORMAL gate: cross-AI review of the exact staged diff, sha256-bound verdict, commit through the real pre-commit gate. One rejection ABORTS the coronation — winning earns a candidacy, not a bypass. The full cycle was proven live on day one. The same release folded four field fixes back in: GitHub Actions pinned by SHA (#1374), lint configs and gate generated only when the tool is present (#1357), diff clipping declared as the pipeline's own doing (#1381), and file splits no longer masquerading as coverage deletion — moved lines are a deterministic signal (#1364).
